...
export default class HomePageTable extends Event {
	...
	rowClicked(e, ele) {
		...
		let _OrderType = e.target.getAttribute("data-trade-type");
		if (_type === "Admin") {
			Swal.fire({
				icon: 'error',
		...
		} else if (_OrderType === "Bid") {
			Swal.fire({
				icon: 'error',
				title: 'Permission Denied',
				text: 'Bidding will be open soon!...',
				// footer: '<a href="">Why do I have this issue?</a>'
			})
		} else {
		...
		if (e.target.type === 'button') {
			...
			for (let sib of childNodes) {
				if (sib.tagName === "SELECT") {
					...
					AjaxUtil.PostData("/Online-Orders/trading-status", { "commid": commodity })
						.then(response => {
							if (response.ok) {
								window.open("/Online-Orders/openTradingWindow?commid=" + commodity + "&buyrate=" + rate + "&type=" + type + "&selectType=" + selectedValue + "&BooRepAll=0&oper=1&sview=1&detailview=1", "trading-view", "menubar=0,scrollbars=1,resizable=0,width=800,height=600,left=1000,top=0")
							} else {
								response.text()
									.then(data => {
										Swal.fire({
	...
	openOrder(e, ele) {
		e.preventDefault();
		let openOrder = new Dialoge({ src: "/Online-Orders/get-order-details" })
		openOrder.showModal();
	}

	pendOrder(e, ele) {
		e.preventDefault();
		let pendingOrder = new Dialoge({ src: "/Online-Orders/get-pending-details" })
	...
	apiorderdeatils(e) {
		e.preventDefault();
		let apiorderdeatils = new Dialoge({ src: "/Online-Orders/api/apiorderdeatils", width: "70%" })
